...
public class ObterDoramaCompletoUseCaseTests
{
    ...
    [Fact]
    public async Task Deve_PreencherDataFim_QuandoDoramaNaoEstaEmExibicao()
    {
        var dorama = DoramaCompletoFactory.Criar();
        var temporada = new Temporada(
            id: Guid.NewGuid(),
        ...
        Assert.NotNull(resultado);
        Assert.NotNull(resultado!.Temporadas[0].DataFim);
    }

    [Fact]
    public async Task Deve_DeixarDataFimNula_QuandoDoramaEstaEmExibicao()
    {
        var dorama = DoramaCompletoFactory.Criar();
        var temporada = new Temporada(
            id: Guid.NewGuid(),
        ...
        Assert.NotNull(resultado);
        Assert.Null(resultado!.Temporadas[0].DataFim);
    }
}